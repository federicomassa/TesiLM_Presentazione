...
\item \textbf{anelastiche}: almeno uno dei due protoni viene frammentato
...
\item Ogni particella di pile-up viene simulata solo se 
...
\includegraphics[width=\textwidth]{Tracking/nPixHits_abseta3}
\end{column}
\begin{column}{0.41\textwidth}
\includegraphics[width=\textwidth]{Tracking/nSCTHits_abseta3}
\end{column}
\begin{column}{0.09\textwidth}
\end{column}
\end{columns}
...
\end{column}
\begin{column}{0.41\textwidth}
\includegraphics[width=\textwidth]{Tracking/nSiHits_abseta3}
\end{column}
\begin{column}{0.41\textwidth}
\begin{itemize}
...
\item[\color{black}--] Per ogni parametro, fit gaussiano della distribuzione di $x_{ricostruito} - x_{generato}$ in diversi intervalli di $\eta$
	\begin{itemize}
...
\centering
\includegraphics[width=.65\textwidth]{Tracking/pionMatchingDR3}
...
\item[\color{black}--] Piccolo effetto osservato all'aumentare del pile-up
\end{itemize}
\end{frame}
...
\item Le risoluzioni rispettano i requisiti desiderati
\end{itemize}
...
\item Stesso campione generato di $H \rightarrow 4\mu$ per i confronti
\end{itemize}
...
\setbeamercolor{local structure}{fg=black}
\begin{itemize}
\item[\color{black}--] \small Si ipotizza 100\% di efficienza di identificazione dei muoni
...
\begin{itemize}
\item \small Segnale (S) e fondo (B) definiti come il numero di eventi in una regione ampia
...
\item<2-> I layout considerati sono robusti rispetto al pile-up;
\item<3-> In generale, i layout ``inclined'' mostrano prestazioni migliori rispetto a quello
``extended'', ma quest'ultimo soffre della mancanza di un algoritmo di ricostruzione ottimizzato.
\item<4-> Abbiamo misurato il canale $H \rightarrow ZZ^{*} \rightarrow 4\mu$ con
...
\centering
\includegraphics[width=.9\textwidth]{innerdetector_sideways_2}
\end{frame}
%----------------------------------------------
