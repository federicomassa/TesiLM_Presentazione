...
\onslide<2->{
    \path let       \p1 = ($(fig.north west)$),
			...
			\p3 = ($(\p1) - (0,\n{figheight}) + (.22*\n{figwidth},.25*\n{figheight})$)  in
	node (rect) at (\p3) [draw,red,thick,minimum width=.05*\n{figwidth},minimum height=.18*\n{figheight}] {};
	%\draw<2->[red] (2.45,1) rectangle (3,2);    
    \node[anchor=south west,inner sep=0] (zoom) at (4,-1.9) {\includegraphics[cfbox=red 4pt,width=.2\textwidth]{InclZoom}};
...
%coordinate (zoomEdge) at (\x6, \y6);
    ...
    \draw [red,thick] (rect.south east) -- ($(zoomEdge)$);
